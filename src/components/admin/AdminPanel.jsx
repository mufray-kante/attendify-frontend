...
export default function AdminPanel() {
    const [pin, setPin] = useState(''); // This is your session PIN
    ...
    const createSession = async () => {
        try {
            ...
            // The backend returns 'pin' in res.data
            setPin(res.data.pin);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className='p-6'>
            ...
            {/* Only show QR code if pin exists */}
            {pin && (
                <div className='mt-4'>
                    <h3 className='mb-2'>Session PIN / QR Code</h3>
                    <QRCodeCanvas value={pin} size={150} />
                    <p className='mt-2 text-lg font-mono'>{pin}</p>
                </div>
            )}
        </div>
    );
